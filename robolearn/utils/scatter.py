...
# def scatter_logs(logs, x_key, y_key, last_log_idx=None):
#     keys = []
#     y_keys = y_key.split("/")
#     for name, log in logs.items():
#         key_in_log = False
...
def main(log_path, filename, x_key, filt_key, size, vmin, vmax, plot, smooth):
    ...
    y_keys = [
        "val_real0_pose_error",
        # "val_real1_pose_error",
        # "val_real2_pose_error",
        # "val_real3_pose_error",
    ]

    for y_key in y_keys:
        last_log_idx = config.get(y_key, None)
        print(y_key)
        last_exp_info = print_logs(logs, x_key, y_key, last_log_idx)
        if plot:
            plt.scatter(last_exp_info.keys(), last_exp_info.values())
